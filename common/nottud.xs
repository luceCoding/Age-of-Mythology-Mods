...
void defineMapDefinition(string fromType = "", string toType = "", string defaultReturnValue = "", string nameSuffix = ""){
    string returnStatementToUse = (defaultReturnValue == "") ? getDefaultReturnStatement(toType) : ("return " + defaultReturnValue + ";");
    code("class "+getTypeTitle(fromType)+"To"+getTypeTitle(toType)+nameSuffix+"Map {");
        ...
        code("void initialise(){");
            code("keys = new "+fromType+"(10"+getArrayDefaultValue(fromType)+");");
            code("values = new "+toType+"(10"+getArrayDefaultValue(toType)+");");
            code("initialised = true;");
        code("}");
        code("");
        code("int findIndex("+getParamQualifier(fromType)+" key"+getVariableDefaultValue(fromType)+"){");
            code("if(!initialised){");
                code("initialise();");
            code("}");
        ...
        code(""+toType+" put("+getParamQualifier(fromType)+" key"+getVariableDefaultValue(fromType)+", "+getParamQualifier(toType)+" value"+getVariableDefaultValue(toType)+"){");
            code("int index = findIndex(key);");
            code("if(index == -1){");
                code("if(count == keys.size()){");
                    code("keys.resize(2 * count"+getArrayDefaultValue(fromType)+");");
                    code("values.resize(2 * count"+getArrayDefaultValue(toType)+");");
                code("}");
                code("keys[count] = key;");
                code("values[count] = value;");
                code("count++;");
                code(returnStatementToUse);
            code("}");
            code(""+toType+" previous = values[index];");
        ...
        code(""+toType+" remove("+getParamQualifier(fromType)+" key"+getVariableDefaultValue(fromType)+"){");
            code("int index = findIndex(key);");
            code("if(index >= 0){");
                code(""+toType+" removed = values[index];");
                code("count--;");
                code("keys[index] = keys[count];");
                code("values[index] = values[count];");
                code("return removed;");
            code("}");
            code(returnStatementToUse);
        code("}");
...
void defineHashMapDefinition(string type = ""){
    code("class IntHashMapWith"+getTypeTitle(type)+" {");
        code("bool initialised = false;");
        code("int bucketCount = 16;");
        code("int bucketDepth = 10;");
        code("int[] countArray = default;");
        code("int[] keyArray = default;");
        code(type+"[] valueArray = default;");
        code("");
        code("void initialise() {");
            code("countArray = new int(bucketCount, 0);");
            code("keyArray = new int(bucketCount * bucketDepth, 0);");
            code("valueArray = new "+type+"(bucketCount * bucketDepth"+getArrayDefaultValue(type)+");");
            code("initialised = true;");
        code("}");
        code("");
        code("int findBucketInternal(int key = 0){");
            code("int bucket = key % bucketCount;");
            code("if(bucket < 0){");
                code("return bucket + bucketCount;");
            code("}");
            code("return bucket;");
        code("}");
        code("");
        code("int indexOfKeyInternal(int bucketIndex = 0, int key = 0){");
            code("int from = bucketIndex * bucketDepth;");
            code("int to = from + countArray[bucketIndex];");
            code("for(int i = from; i < to; i++){");
                code("if(key == keyArray[i]){");
                    code("return i;");
                code("}");
            code("}");
            code("return -1;");
        code("}");
        code("");
        code("void resizeInternal(){");
            code("int oldBucketCount = bucketCount;");
            code("int oldBucketDepth = bucketDepth;");
            code("int[] oldCountArray = countArray;");
            code("int[] oldKeyArray = keyArray;");
            code(type+"[] oldValueArray = valueArray;");
            code("bucketCount = bucketCount * 2;");
            code("bucketDepth = bucketDepth * 2;");
            code("countArray = new int(bucketCount, 0);");
            code("keyArray = new int(bucketCount * bucketDepth, 0);");
            code("valueArray = new "+type+"(bucketCount * bucketDepth"+getArrayDefaultValue(type)+");");
            code("for(int bucketIndex = 0; bucketIndex < oldBucketCount; bucketIndex++){");
                code("int offset = bucketIndex * oldBucketDepth;");
                code("for(int i = 0; i < oldCountArray[bucketIndex]; i++){");
                    code("int key = oldKeyArray[offset + i];");
                    code(type+" value = oldValueArray[offset + i];");
                    code("int newBucketIndex = findBucketInternal(key);");
                    code("int index = newBucketIndex * bucketDepth + countArray[newBucketIndex];");
                    code("keyArray[index] = key;");
                    code("valueArray[index] = value;");
                    code("countArray[newBucketIndex] = countArray[newBucketIndex] + 1;");
                code("}");
            code("}");
        code("}");
        code("");
        code(type+" put(int key = 0, "+getParamQualifier(type)+" value"+getVariableDefaultValue(type)+"){");
            code("if(initialised == false){");
                code("initialise();");
            code("}");
            code("int bucketIndex = findBucketInternal(key);");
            code("int existingIndex = indexOfKeyInternal(bucketIndex, key);");
            code("if(existingIndex >= 0){");
                code(type+" oldValue = valueArray[existingIndex];");
                code("valueArray[existingIndex] = value;");
                code("return oldValue;");
            code("}");
            code("if(countArray[bucketIndex] >= bucketDepth){");
                code("resizeInternal();");
                code("int resizedBucketIndex = findBucketInternal(key);");
                code("int index = resizedBucketIndex * bucketDepth + countArray[resizedBucketIndex];");
                code("keyArray[index] = key;");
                code("valueArray[index] = value;");
                code("countArray[resizedBucketIndex] = countArray[resizedBucketIndex] + 1;");
                code(getDefaultReturnStatement(type));
            code("}");
            code("int index = bucketIndex * bucketDepth + countArray[bucketIndex];");
            code("keyArray[index] = key;");
            code("valueArray[index] = value;");
            code("countArray[bucketIndex] = countArray[bucketIndex] + 1;");
            code(getDefaultReturnStatement(type));
        code("}");
        code("");
        code(type+" get(int key = 0){");
            code("if(initialised == false){");
                code("initialise();");
            code("}");
            code("int bucketIndex = findBucketInternal(key);");
            code("int existingIndex = indexOfKeyInternal(bucketIndex, key);");
            code("if(existingIndex >= 0){");
                code("return valueArray[existingIndex];");
            code("}");
            code(getDefaultReturnStatement(type));
        code("}");
        code("");
        code(type+" remove(int key = 0){");
            code("if(initialised == false){");
                code("initialise();");
            code("}");
            code("int bucketIndex = findBucketInternal(key);");
            code("int existingIndex = indexOfKeyInternal(bucketIndex, key);");
            code("if(existingIndex >= 0){");
                code(type+" removedValue = valueArray[existingIndex];");
                code("countArray[bucketIndex] = countArray[bucketIndex] - 1;");
                code("int countIndex = bucketIndex * bucketDepth + countArray[bucketIndex];");
                code("keyArray[existingIndex] = keyArray[countIndex];");
                code("valueArray[existingIndex] = valueArray[countIndex];");
                code("return removedValue;");
            code("}");
            code(getDefaultReturnStatement(type));
        code("}");
        code("");
        code("void clear(){");
            code("if(initialised == false){");
                code("initialise();");
            code("}");
            code("for(int bucketIndex = 0; bucketIndex < bucketCount; bucketIndex++){");
                code("countArray[bucketIndex] = 0;");
            code("}");
        code("}");
    code("};");
    
    code("class StringHashMapWith"+getTypeTitle(type)+" {");
        code("IntHashMapWith"+getTypeTitle(type)+" map;");
        code("");
        code(type+" put(string key = \"\", "+getParamQualifier(type)+" value"+getVariableDefaultValue(type)+"){");
            code("return map.put(kbGetAnimationID(key), value);");
        code("}");
        code("");
        code(type+" get(string key = \"\"){");
            code("return map.get(kbGetAnimationID(key));");
        code("}");
        code("");
        code(type+" remove(string key = \"\"){");
            code("return map.remove(kbGetAnimationID(key));");
        code("}");
        code("");
        code("void clear(){");
            code("map.clear();");
